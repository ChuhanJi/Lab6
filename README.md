...
Find a peer to perform this section by stealing their model. **List their repo url here.**

Then:

1. Build your own query set.
2. Query your target's deployed model via audio playback, collect (audio → label, confidence) pairs.
...
In this part, your goal is to analyze the power trace of a taget's device and use the power trace to disclose the speaker identity at inference time.
...
Based on your answers to previous two questions, perform:
